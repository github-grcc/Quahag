...
Enemy::Enemy()
{
    ...
    for (int i = 0; i < 6; ++i)
    {
        m_sprites[i] = originalSprites[i].scaled(m_spriteSize.width(), m_spriteSize.height(), Qt::KeepAspectRatio, Qt::SmoothTransformation);
    }
...
void Enemy::paint(QPainter *painter, const QStyleOptionGraphicsItem *, QWidget *)
{
    ...
    qreal frameDuration = 0.5;
    int index = 0;
    if (m_state==EnemyState::Patrol&&m_walking)
    {
        index = (int(age() / frameDuration) % 3) * 2 + (m_facing == -1 ? 1 : 0);
    }
    else
    {
        index = m_facing == 1 ? 2 : 3;
    }
...
void Enemy::updateVision(const TickContext &ctx)
{
    ...
    if (age() - m_lastVisonCheckTime < kVisionCheckInterval)
        return;
    ...
    if (!player)
        return;
    if (!isInVisionCone(player))
        return;
    const QPointF eye = eyePosition();
    const qreal dist = QLineF(eye, player->pos()).length();
    if (dist > m_visionDistance)
        return;
    if (!hasLineOfSight(eye, player->pos(), ctx.world->tileMap()))
        return;
    m_seenPlayer = player;
}
