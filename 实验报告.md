...
**自选题目：桌面端“平台跳跃/动作”类游戏——“圆哈镇3022（Quahag）”**

**具体描述：**
本项目基于C++语言和Qt图形框架，开发一款名为”圆哈镇3022（Quahag）”的桌面端2D平台跳跃/动作游戏。故事设定在3022年——玩家操控外出归来的”耄耋”返回故乡”圆哈镇”，却发现家园已被名为”胖宝宝”的敌人占领。为了夺回属于自己的王座，耄耋穿行于复杂的博古架地形之间，与巡逻的胖宝宝周旋战斗。途中散落着”哈基米特饮”（短暂全属性加速）和”口香糖”（恢复生命值）等道具，合理利用它们是通关的关键。
...
resources.qrc
core/
  gameloop.h / .cpp      — 游戏循环（QTimer 驱动）
  tickcontext.h           — TickContext + WorldEvents + InputState
  entitytypes.h           — EntityKind / Faction / ZLayer 枚举
  sound.h                 — 音效（paplay）
entities/
  actoritem.h / .cpp      — 实体抽象基类（继承 QGraphicsObject）
  player.h / .cpp         — 玩家（3 状态机 + 攻击）
  enemy.h / .cpp          — 敌人（5 状态 AI + 视觉系统）
  bullet.h / .cpp         — 子弹
  throne.h / .cpp         — 王座（胜利条件）
  hachimi.h / .cpp        — 哈基米（加速道具）
  chutty.h / .cpp         — 口香糖（回血道具）
  particle.h / .cpp       — 粒子（爆炸 / 灰尘）
  claweffect.h / .cpp     — 爪痕特效
  trailghost.h / .cpp     — 拖尾残影
  healcross.h / .cpp      — 治疗十字粒子
graphics/
  camera2d.h / .cpp       — 摄像机（弹簧跟随 / 抖动 / 缩放脉冲）
  parallaxbackground.h / .cpp — 视差背景（5 层）
  tilelayeritem.h / .cpp  — 瓦片图层（视口裁剪）
world/
  gameworld.h / .cpp      — 实体中枢（所有权 / 索引 / 生命周期）
  gamescene.h / .cpp      — QGraphicsScene 桥接
  tilemap.h / .cpp        — 瓦片地图（碰撞 / 单向墙）
  levelbuilder.h / .cpp   — 关卡生成器
ui/
  gameview.h / .cpp       — QGraphicsView（状态机 / 输入 / HUD）
rsc/
  map3.txt                — 关卡地图数据
  sprites/                — 精灵图片
  sounds/                 — 音效文件
...
游戏采用四状态机（`GameState`枚举）管理整体流程，由`GameView`类统一协调：
